...
function BulkInvite({ inviteTemplate, registrationLink, communities, neighborhoodId, setNeighborhoodId, selectedCommunity }: SharedProps) {
  ...
  // Tracks the last auto-generated message so a community switch can tell
  // "untouched" apart from "the admin edited this" — see the effect below.
  ...
  useEffect(() => {
    ...
    // Bug this fixes: previously this only ran once ever (guarded by
    // `!message`), so {{community_line}} never reflected a *later* dropdown
    // change — it stayed stuck on whichever community was selected at the
    // very first render.
