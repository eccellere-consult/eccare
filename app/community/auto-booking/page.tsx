...
import { Car, Phone, Plus, Trash2, IndianRupee, MessageCircle } from 'lucide-react';
 ...
 *  sends themselves, and the driver confirms by calling back. */
...
export default function AutoBookingPage() {
  ...
  function bookMessage(driver: Driver): string {
    const tripLabel = tripType === 'drop' ? 'One-way drop' : 'Wait & return';
    return [
      'Hello, I need to book your auto through EC.',
      `Trip: ${tripLabel}`,
      `Pickup: ${pickup || '(please confirm)'}`,
      `Drop: ${drop || '(please confirm)'}`,
      rateCard ? `Community rate: ₹${rateCard.perKmRate}/km, ₹${rateCard.perMinWaitRate}/min waiting.` : '',
      'Please call to confirm you can take this trip. Thank you!',
    ]
      .filter(Boolean)
      .join('\n');
  }
  ...
  return (
    <CommunityPageFrame
      title="Auto Booking"
      subtitle="Book a trusted local auto-rickshaw for an elder — drop-off, or wait and return."
      loading={loading}
      error={error}
    >
      {rateCard ? (
        <Card className="border-accent-100 bg-accent-50">
          <CardContent className="flex flex-wrap items-center gap-4 py-4">
            ...
            <p className="text-sm text-text">
              ...
              — agree the exact fare with the driver before the trip.
            </p>
            {canManage && (
        ...
        <Card>
          <CardContent className="flex items-center justify-between gap-3 py-4">
            <p className="text-sm text-text-secondary">No rate card set yet.</p>
            <Button size="sm" onClick={() => { setRateForm({ perKmRate: '', perMinWaitRate: '' }); setShowRateForm(true); }}>Set rates</Button>
          </CardContent>
        </Card>
      ) : null}
            ...
            <CardContent className="pt-6">
              ...
              {bookingDriverId === driver.id && (
                <div className="mt-3 flex flex-col gap-2 border-t border-border pt-3">
                  ...
                  <a
                    href={waLink(driver.phone, bookMessage(driver))}
                    target="_blank"
                  ...
                  <p className="text-xs text-text-secondary">
                    Opens WhatsApp with your trip details filled in — the driver will call to confirm.
                  </p>
                </div>
              )}
            </CardContent>
