...
/** Appends one photo/video to the inspection's mediaPaths — called once per
 *  file, since the checklist form uploads them one at a time as they're taken. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const inspection = await prisma.propertyInspection.findUnique({ where: { id } });
  ...
  try {
    ...
    console.error('Property inspection media upload error:', message);
