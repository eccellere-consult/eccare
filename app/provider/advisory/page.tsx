...
export default async function ProviderAdvisoryPage() {
  ...
  if (provider.category !== 'legal_help' && provider.category !== 'insurance') {
    return (
      <div>
        {backLink}
        <h1 className="mt-3 text-2xl font-bold text-text">Advisory profile &amp; consultations</h1>
        <Card className="mt-4">
          <CardContent className="flex items-start gap-4 py-8">
            <Scale className="h-6 w-6 shrink-0 text-text-secondary" />
            <div>
              <p className="font-bold text-text">This page is for Legal Help / Insurance accounts</p>
              <p className="mt-1 text-sm text-text-secondary">
                Your account is registered under &ldquo;{provider.category}&rdquo;. Advisory profile and
                consultation management is specific to those categories — if that&rsquo;s a mistake,
  ...
  const advisoryExpert = await prisma.advisoryExpert.findUnique({ where: { providerId: provider.id } });
